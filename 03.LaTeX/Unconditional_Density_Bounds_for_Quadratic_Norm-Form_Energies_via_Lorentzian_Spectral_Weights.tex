...
\medskip\noindent\textbf{Reproducibility.}  The values of $I_{\mathbf{n}}$ were computed by ARB quadrature (\texttt{acb.integral}, 256-bit precision) with the integrand
\[
...
The ratio $I_{\mathbf{n}}/b_M$ is bounded and stabilizing, confirming $C_M \leq C$ uniformly.  For $n_M = 2$, the ratio $I_{\mathbf{n}}/b_M^2$ similarly stabilizes near $1550$.  Each entry $I_{\mathbf{n}}$ in the table corresponds to a resonance vector with $n_M = 1$ and all other components zero; the integral was evaluated by ARB quadrature (\texttt{acb.integral}, 256-bit precision, $T = 2000$).
