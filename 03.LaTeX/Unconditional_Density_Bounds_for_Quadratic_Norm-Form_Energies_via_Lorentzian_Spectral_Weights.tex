...
\end{tabular}
\end{center}
Each additional inactive $J_0$ factor reduces the resonance integral, converging to a limit as $M \to \infty$.  The values of $I_{\mathbf{n}}$ were computed by ARB-rigorous strip decomposition (\texttt{acb\_calc\_integrate}, 256-bit precision, $T = 2000$): the domain is partitioned at the certified zeros of each $J_1$ and $J_0$ factor so that \texttt{acb\_calc\_integrate} is applied only on gap intervals where the integrand is analytic.  Active indices $(n_1, n_2, n_3) = (1, 1, -1)$; the remaining $M - 3$ indices are inactive.
\end{remark}
