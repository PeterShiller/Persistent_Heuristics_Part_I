...
\medskip
Combining Cases~1 and~2: for the eight small discriminants, the Ces\`aro variance exceeds the threshold for all $T > 0$ (Case~2, grid scan).  For $d \geq 14$, the three-region argument of Case~1 (Regions~1--3 above) certifies the inequality for all $T > 0$ at the worst-case five-zero configuration; since every actual configuration with $d \geq 14$ has at least as many zeros and a no smaller $W_1(L)$, the margin can only improve.
\end{proof}
