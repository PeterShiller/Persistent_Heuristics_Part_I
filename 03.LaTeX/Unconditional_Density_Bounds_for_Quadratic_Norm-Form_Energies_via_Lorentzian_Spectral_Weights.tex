...
\]
for all $T > 0$, where $c_0 = (\pi-1)/(2\pi)$.  For $q \geq 17$ and $T = 11$, Theorem~\ref{thm:BMOR} guarantees at least four $\gamma'_{\mathrm{unk}}$ zeros below height $11$ (BMOR gives $N \geq 4.79$ at $q = 17$; for $q \geq 20$, at least five), giving $W_2(L) \geq 4\,w^-(11)^2 = 4(8/488)^2 = 256/238144$.  Therefore $c_0 W_2(L) \geq 256(\pi-1)/(476288\pi) > 3.66 \times 10^{-4}$, while $(S_\zeta^*)^2/d \leq (0.04871)^2/14 < 1.70 \times 10^{-4}$, a margin of $2.2\times$ or better for all $d \geq 14$.  For a representative five-$\gamma'_{\mathrm{unk}}$-zero configuration at $q \geq 20$ (equally spaced in $[6,11]$), the interference constant evaluates to $\mathcal{I} = 5.47 \times 10^{-3}$, so the full Ces\`aro variance exceeds the threshold for all $T > 3.1$.
...
\smallskip\noindent The three regions overlap and certify the inequality for all $T > 0$ at the representative configuration; every actual configuration with $d \geq 14$ has at least four zeros (by BMOR at $T = 11$; five for $q \geq 20$) and a diagonal margin of $2.2\times$ or better, so the inequality holds universally.
...
\medskip
Combining Cases~1 and~2: for the eight small discriminants, the Ces\`aro variance exceeds the threshold for all $T > 0$ (Case~2, grid scan).  For $d \geq 14$, the diagonal bound $c_0 W_2 \geq 2.2 \times (S_\zeta^*)^2/d$ (Case~1) holds universally over all four-or-more-zero configurations, and the three-region argument certifies the full bilinear form for all $T > 0$ at the representative configuration.
\end{proof}
