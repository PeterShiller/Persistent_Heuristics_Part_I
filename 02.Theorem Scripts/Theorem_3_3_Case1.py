...
All three regions are certified entirely in ARB interval arithmetic.  They
overlap at T = 0.01 and T = 50, jointly covering all T > 0.
...
-----------------------
  (a) All function evaluations use ARB ball arithmetic (arb.sin, arb arithmetic);
      no mpmath or float arithmetic is used inside any certified computation.
...
def h_arb(T_arb):
    ...
    Returns a certified ARB ball.  The sinc(0) limit (j == k, T -> 0) is
    handled automatically by ARB's sin/division: at T = 0 the argument of sin
    is 0 and sin(0)/0 is resolved as 1 by ARB's interval containment.
    """
    total = arb(0)
    for p in PAIRS:
        if p[0] == "diag":
...
def certify_region2(C):
    ...
    for _ in range(GRID_STEPS):
        h = h_arb(arb(str(T_float)))
        if grid_min is None or bool(h < grid_min):
            grid_min   = h
            grid_min_T = T_float
        T_float = round(T_float + 5e-4, 7)
